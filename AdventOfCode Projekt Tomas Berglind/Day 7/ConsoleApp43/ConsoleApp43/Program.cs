...
//Console.WriteLine("Den bästa positionen, som drar minst fuel för de {2} st krabborna är plats {0}, där det dras {1} Fuel", bestPos, bestFuel, crabPositions.Length); //342730
//Console.ReadKey();



//Slut del 1 (allt avkommenterat)
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
//Start del 2
...
for (int i = leftPos; i < rightPos; i++) //Går genom varje postion
{
    ...
    for (int j = 0; j < crabPositions.Length; j++) //Går genom varje krabba
    {
        int extraFuelCost = 0; //Del 2, extrakostnaden nollställs efter varje krabba har rört sig sina steg

        if (crabPositions[j] > i) //Lägger till mellanskillnaden från nuvarande position i första loopen och nuvarande krabba (andra loopen)
        {
            fuel += (crabPositions[j] - i) + AddExtraFuel(crabPositions[j] - i);
        }
        else if (crabPositions[j] < i)
        {
            fuel += (i - crabPositions[j]) + AddExtraFuel(i - crabPositions[j]);
        }
    }
...
//Tar in mellanskillnaden och returnerar hur många extra Fuel det kostar i del 2
static int AddExtraFuel(int difference)
{
    int extraFuel = 0;
...
Console.WriteLine("Den bästa positionen, som drar minst fuel för de {2} st krabborna är plats {0}, där det dras {1} Fuel", bestPos, bestFuel, crabPositions.Length);
Console.ReadKey();
